/**
  Representation of a chess board for the
  queens puzzle of size *ranks*.
 */

public class BoardForQueensPuzzle {
    private int lastRankFilled;
    private int[] filesWithQueens;  //as explained in my homework, a one-dimensional array would suffice
    ...
    public boolean lastIsNg() {
        for(int rank = 0; rank < lastRankFilled; rank++) {
            if (filesWithQueens[rank] == filesWithQueens[lastRankFilled]) 
              return true;
            if (Math.abs(rank - lastRankFilled) == Math.abs(filesWithQueens[rank] - filesWithQueens[lastRankFilled])) 
              return true;
        }
        return false;
    }
